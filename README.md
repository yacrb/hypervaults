...
For event-specific flags and challenge credentials, copy the example secret files and point Compose at the ignored directory:

```bash
cp -r secrets.example secrets
# edit secrets/*.txt for the workshop, then set this in .env:
SECRETS_DIR=./secrets
```

The repository defaults to `SECRETS_DIR=./secrets.example` so a local training stack can start without real secrets. The values in `secrets.example/` and `.env.example` are fake CTF values only.
...
The htpasswd entry for Mailpit Basic Auth is generated by the nginx `entrypoint` from Docker secrets or fallback environment variables.
