...
export default function SignupPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const response = await signup(email, password, turnstileToken);
    ...
    <main className="min-h-screen bg-cloud">
      ...
      <section className="mx-auto flex max-w-md flex-col px-4 py-12 sm:px-6">
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-soft">
          ...
          <p className="mt-2 text-sm text-slate-600">Start a private document vault for local development testing.</p>
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            ...
            <label className="block">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm text-slate-900"
              />
              <span className="mt-1.5 block text-xs text-slate-500">Use at least 12 characters.</span>
            </label>
            ...
            >
              {isSubmitting ? <Loader2 className="animate-spin" aria-hidden="true" size={17} /> : <UserPlus aria-hidden="true" size={17} />}
              Create account
            </button>
          </form>
          ...
          <p className="mt-5 text-sm text-slate-600">
            ...
            <Link href="/login" className="font-semibold text-vault hover:text-teal-800">
              Log in
            </Link>
          </p>
        </div>
